...
class Demo(commands.Cog):
    ...
    async def demo_ddv(self, interaction: discord.Interaction):
        ...
        embed.add_field(name=":one: 指令: /select_stream",
                        value="直接送出即可，選擇想要查詢的頻道，機器人會回傳該實況主最近的三個 VOD。目前列表以子午藝人為多數。",
                        inline=False)
        embed.add_field(name=":two: 指令: /time_travel",
                        value="需要手動輸入實況主的 Twitch 帳號，也就是頻道連結: twitch.tv/斜線後面這段。再逐項輸入日期時間即可。須注意時間為24小時制。",
                        inline=False)
        embed.add_field(name=":three: 右鍵選單指令 :new:",
                        value="邀請 DDV 到頻道或私聊後，對任意訊息點右鍵->應用程式->可以選擇直接換算KSP/SEKI在該訊息當下的 VOD 連結。此訊息只會給你自己看到，不必擔心吵到別人。",
                        inline=False)
